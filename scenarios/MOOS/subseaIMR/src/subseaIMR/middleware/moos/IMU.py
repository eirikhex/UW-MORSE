...
class IMUNotifier(AbstractMOOS):
    ...
    def default(self,  ci = 'unused'):
        ...
        # post angular rates
        self.notify('imu_gyro_p', vel[1], cur_time)
        self.notify('imu_gyro_q', vel[0], cur_time)
        self.notify('imu_gyro_r', -vel[2], cur_time)
    
        # post accelerations
        self.notify('imu_acc_x', acc[1], cur_time)
        self.notify('imu_acc_y', acc[0], cur_time)
        self.notify('imu_acc_z', -acc[2], cur_time)
        
        # post magnetometer values
        self.notify('imu_mag_x', mag[1], cur_time) # North component
        self.notify('imu_mag_y', mag[0], cur_time) # East component
